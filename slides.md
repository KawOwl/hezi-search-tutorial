...
查询分析结果:
{
  brand: [["苹果", 1.0], ["apple", 0.9]], ......    // 品牌向量相似度搜索结果
  categoryFront: [["手机", 1.0], ["智能手机", 0.9]], ......  // 类目向量相似度搜索结果
  categoryRaw: []                             // 未找到后台类目
}

//实际上分数会经过加分处理再给到下一环节，例如使用相似度作为基础分，根据是否大牌、类目权重设置等算出最终分数。
```
......
### 评分计算
```typescript
假设某商品：品牌为苹果，前台类目为智能手机，后台类目为空
意图识别猜测的品牌及匹配度分数：[["苹果", 1.0], ["apple", 0.9]], ......
brand_score = 1.0    // 与["苹果", 1.0] 完全匹配
意图识别猜测的前台类目及匹配度分数：[["手机", 1.0], ["智能手机", 0.9]], ......
catF_score = 0.9     // 与["智能手机", 0.9] 前台类目匹配度高
catR_score = 0.0     // 无后台类目
